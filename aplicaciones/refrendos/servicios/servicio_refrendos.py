...
#: Tope de dia 0 (calibrable por convenio via `TOPE_REFERENDOS`).
...
def paginar_refrendos(
    ...
    Args:
        ...
        tope: Maximo por pagina (None = central `TOPE_REFERENDOS`).
    ...
    tope = tope if tope is not None else int(
        getattr(settings, "TOPE_REFERENDOS", TOPE_REFERENDOS)
    )
    desplazamiento = 0
    if cursor:
